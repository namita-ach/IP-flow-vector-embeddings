...
class FFNNEmbeddings:
    ...
    def create_query_embedding(self, query_type, query_value):
        ...
        temp_embedder = FFNNEmbeddings(sample_flow, self.categorical_cols, self.continuous_cols)
        return temp_embedder.get_ffnn_embeddings(embedding_dim=128)
    
    def get_ffnn_embeddings(self, embedding_dim=128): 
        # Prepare data
...
def main(): 
    ...
    # Initialize FFNN embeddings
    ffnn_embedder = FFNNEmbeddings(df, categorical_cols, continuous_cols)
    
    plots_dir = "FFNNResults"
    os.makedirs(plots_dir, exist_ok=True)
    
    print("Processing FFNN embedding method")
    
    # Generate embeddings
    embeddings = ffnn_embedder.get_ffnn_embeddings(embedding_dim=128)
    ...
    tracker = FAISSPerformanceTracker("FFNN")
    ...
    for num_ops in [2500, 5000, 7500, 10000, 20000, 30000]:
        ...
        if num_ops == 10000:
            ...
            try:
                tcp_query = ffnn_embedder.create_query_embedding("protocol", "TCP")
            ...
            try:
                ip_query = ffnn_embedder.create_query_embedding("ip", "192.168")
            ...
            try:
                port_query = ffnn_embedder.create_query_embedding("port", "80")
    ...
    plot_filename = f"{plots_dir}/FFNN_Performance.png"
